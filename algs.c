...
void algorithm2(size_t c, size_t n, size_t m, double neg_g_invh_gt[c][c], double neg_s[c][n], double neg_w[c], double neg_invh_f[m][n], double neg_invh_gt[m][c], double x[n], double invq[c][c], uint8_t a_set[c], double y[c], double v[c], double temp1[c], double temp2[c][c], double temp3[m], double u[m]) {
    eye(c, invq);
    ...
    compute_u(m, n, c, neg_invh_f, x, neg_invh_gt, y, temp3, u);
}
